...
import { getChatDb, schema } from "./src/db/client";
...
const keyStore = new Map<number, { ed25519_pub: Uint8Array; x25519_pub: Uint8Array }>();

function loadKeys(userId: number): { ed25519_pub: Uint8Array; x25519_pub: Uint8Array } | null {
    const cached = keyStore.get(userId);
    if (cached) return cached;
    const db = getChatDb();
    const row = db.select().from(schema.identityKeys).where(eq(schema.identityKeys.user_id, userId)).get();
    if (!row) return null;
    const keys = {
        ed25519_pub: b64ToBytes(row.ed25519_pub),
        x25519_pub: b64ToBytes(row.x25519_pub),
    };
    keyStore.set(userId, keys);
    return keys;
}
...
if (import.meta.main) {
    const server = Bun.serve<{ userId: number }>({
        ...
        websocket: {
            open(ws) {
                ...
                deliverPending(userId, (uid) => loadKeys(uid)?.x25519_pub ?? null, (uid) => loadKeys(uid)?.x25519_pub ?? null);
            },
    ...
    async function handleClientFrame(userId: number, ws: any, frame: any): Promise<void> {
        if (frame.t === "dm.send") {
            ...
            handleDmDelivered(frame.ids);
        } else if (frame.t === "dm.read" && typeof frame.conv === "string") {
            handleDmRead(userId, ws, frame);
        }
    }
    ...
    async function handleDmSend(userId: number, ws: any, frame: any): Promise<void> {
        ...
        const senderKeys = loadKeys(userId);
        const recipientKeys = loadKeys(frame.to);
        ...
        try {
            const result = db.insert(schema.messages)
                ...
                const mid = (result as any).lastInsertRowid as number;
    ...
    function handleDmDelivered(ids: number[]): void {
        const db = getChatDb();
    ...
    function handleDmRead(userId: number, ws: any, frame: any): void {
    ...
    // invalidate key cache when keys are updated via POST /keys
    // (simple: just clear on every new connection; a more efficient approach
    //  would use an event emitter from the keys route)
    setInterval(() => keyStore.clear(), 60_000);
