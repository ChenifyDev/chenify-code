import { countUnreadNotifications, listNotifications, markNotificationsRead } from "../db";
...
export const routes: Bun.Serve.Routes<any, any> = {
    "/api/notifications": {
        GET: async (req) => {
            ...
            return Response.json(listNotifications(me.id, { offset, limit }));
        },
    },
    ...
    "/api/notifications/unread-count": {
        GET: async (req) => {
            const me = await getAuthUser(req);
            if (!me) return jsonError(401, "请先登录");
            return Response.json({ count: countUnreadNotifications(me.id) });
        },
    },
