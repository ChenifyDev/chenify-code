...
function insertOrGetId(username: string, email: string, passwordHash: string): number | null {
    ...
    const result = db.query("INSERT INTO users (username, email, password_hash) VALUES (?, ?, ?)").run(username, email, passwordHash);
    return Number(result.lastInsertRowid);
}

function buildUserIdMap(): Map<string, number> {
    const rows = db.query("SELECT id, username FROM users WHERE username IN (SELECT value FROM json_each(?, '$'))").all(JSON.stringify(SAMPLE_USERNAMES)) as { id: number; username: string }[];
...
async function main() {
    if (reset) {
        ...
        console.log(`已删除 ${ids.length} 个示例账号（含其帖子/评论/收藏/关注关系）`);
    }
    ...
    for (const user of users) {
        user.posts.forEach((post, index) => {
            const existing = db.query("SELECT id FROM posts WHERE user_id = ? AND content = ?").get(idMap.get(user.username)!, post.content) as { id: number } | null;
            let postId: number;
            if (existing) {
                postId = existing.id;
            } else {
                const result = db.query("INSERT INTO posts (user_id, content, created_at) VALUES (?, ?, ?)").run(idMap.get(user.username)!, post.content, post.created_at);
    ...
    for (const user of users) {
        for (const target of user.follows) {
            ...
            if (targetId !== undefined) {
                db.query("INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?, ?)").run(idMap.get(user.username)!, targetId);
            }
        }
        for (const key of user.favorites) {
            const postId = postIds.get(key);
            if (postId !== undefined) {
                db.query("INSERT OR IGNORE INTO favorites (user_id, post_id) VALUES (?, ?)").run(idMap.get(user.username)!, postId);
            }
        }
        for (const key of user.likes) {
            const postId = postIds.get(key);
            if (postId !== undefined) {
                db.query("INSERT OR IGNORE INTO likes (user_id, post_id) VALUES (?, ?)").run(idMap.get(user.username)!, postId);
            }
        }
    }
    ...
    for (const comment of seedComments) {
        ...
        if (postId !== undefined && authorId !== undefined) {
            const existing = db.query("SELECT id FROM comments WHERE post_id = ? AND user_id = ? AND content = ?").get(postId, authorId, comment.content);
            if (!existing) {
                db.query("INSERT INTO comments (post_id, user_id, content, created_at) VALUES (?, ?, ?, ?)").run(postId, authorId, comment.content, comment.created_at);
            }
        }
    }
    ...
    for (const user of users) {
        ...
        const counts = db
            ...
            .get(id, id, id, id, id) as { posts: number; favorites: number; likes: number; following: number; followers: number };
        console.log(`${user.username}(${user.email}) 密码 ${PASSWORD} — 帖子 ${counts.posts} / 收藏 ${counts.favorites} / 点赞 ${counts.likes} / 关注 ${counts.following} / 粉丝 ${counts.followers}`);
    }
